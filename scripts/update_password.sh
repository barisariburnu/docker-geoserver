#!/bin/bash
# this script that allows a user to pass a password or username on runtime.
...
make_hash(){
    NEW_PASSWORD=$1
    (echo "digest1:" && $JAVA_HOME/bin/java -classpath $(find $CLASSPATH -regex ".*jasypt-[0-9]\.[0-9]\.[0-9].*jar") org.jasypt.intf.cli.JasyptStringDigestCLI digest.sh algorithm=SHA-256 saltSizeBytes=16 iterations=100000 input="${NEW_PASSWORD}" verbose=0) | tr -d '\n'
}
